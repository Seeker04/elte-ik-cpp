...
//------------------------------------------
...

	{
		int x = 42;
		std::cout << x << "\n";
	} // <-- itt x megsemmisul, mert lokalis valtozoja volt a blokknak

	std::cout << x << "\n"; // e miatt le se fog fordulni: "'x' was not declared in this scope"
	...
	} // <-- a blokk ketto lokalis valtozoja: p es n megsemmisul

	std::cout << *p << "\n"; // ugyanugy mint elobb: nem fordul le, mert
	                         // ezen a scope-on mar nincs p pointer
	// Amire o mutatott a heap-en (ha n<10 teljesult), az meg ott van
	// leallokalva es mar nem is tudom igy torolni => memory leak!
}
